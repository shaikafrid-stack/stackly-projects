...
export default function AdminServiceRequests() {
  ...
  useEffect(() => {
    userService
      .getAll({ role: 'maintenance_engineer' })
  ...
  const openAssign = (request) => {
    ...
    setEngineerId(request.assigned_engineer_id || '');
  };
  ...
  const submitAssign = async () => {
    ...
    setAssigning(true);
    try {
      await serviceRequestService.update(assignTarget.id, { assigned_engineer_id: Number(engineerId) });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <p className="text-gray-500 text-sm mb-6">Monitor all requests and assign maintenance engineers.</p>
            ...
            <tbody>
              {requests.map((r) => (
                <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4">{r.employee_name}</td>
                  <td className="py-3 px-4">{r.engineer_name || '—'}</td>
                  ...
                  <td className="py-3 px-4 flex gap-3">
                    <button className="text-brand-600 font-medium" onClick={() => openAssign(r)}>
                      Assign
                    </button>
                    <button className="text-gray-500 font-medium" onClick={() => openDetail(r.id)}>
                      View
                    </button>
                  </td>
                </tr>
      ...
      <Modal open={!!assignTarget} onClose={() => setAssignTarget(null)} title="Assign Engineer" maxWidth="max-w-sm">
        <p className="text-sm text-gray-600 mb-3">
          Assign an engineer to: <span className="font-medium">{assignTarget?.issue_title}</span>
        </p>
        <select className="input mb-4" value={engineerId} onChange={(e) => setEngineerId(e.target.value)}>
        ...
        <button className="btn-primary w-full" disabled={assigning} onClick={submitAssign}>
      ...
      <Modal open={!!selected} onClose={() => setSelected(null)} title="Service Request Details">
        {selectedLoading || !selected ? (
          ...
          <div className="space-y-3 text-sm">
            ...
            <p>
              <span className="font-medium">Employee:</span> {selected.employee_name}
            </p>
            <p>
              <span className="font-medium">Engineer:</span> {selected.engineer_name || 'Not yet assigned'}
            </p>
            <div>
                  ...
                  {selected.maintenance_logs.map((log) => (
                    <li key={log.id} className="border border-gray-200 rounded-lg p-3">
                      <p className="text-xs text-gray-400 mb-1">
                        {log.engineer_name} ·{' '}
                        {log.resolved_at ? new Date(log.resolved_at).toLocaleString() : 'In progress'}
                      </p>
